...
class BaseModel(nn.Module):
    ...
    def __init__(self, config, model, backend, **kwargs):
        ...
        # by @alexa
        if "is_llava" in kwargs:
            self.is_llava = kwargs["is_llava"]
        else:
            self.is_llava = False
    ...
    def _gather_intervention_output(
        ...
        if (
            ...
            if self.is_llava:
                crop_idx = int(representations_key.split("#")[-1])
            else:
                crop_idx = None
        ...
        if activations_sources is None:
            ...
            if self.is_llava:
                ...
                for group_id, keys in self._intervention_group.items():
                    if sources[group_id] is None:
                        continue  # smart jump for advance usage only
                    group_get_handlers = HandlerList([])  # @alexa was commented out...
                    for key in keys:
                    ...
                    if len(sources) > 0 and sources[group_id] is not None:
                ...
                for group_id, keys in self._intervention_group.items():
                    ...
                    for key in keys:
                        ...
                        group_get_handlers.extend(get_handlers)
                # EDIT BY @Alexa: I moved the following two lines to outside the loop above and indexed sources with 0
                # BC all sources are identical from what I can tell, and we should only have to call the model once.
                # Could cause problems IDK
                if len(sources) > 0 and sources[0] is not None:
