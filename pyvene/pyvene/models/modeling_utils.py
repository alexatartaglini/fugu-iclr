...
def b_sd_to_bsd(tensor, s):
    ...
    if tensor is None:
        return tensor
    #if len(tensor.shape) == 2:  # @Alexa: need to remove this
    #    return tensor
    b, sd = tensor.shape
    d = sd // s
    return tensor.reshape(b, s, d)
...
def gather_neurons(tensor_input, unit, unit_locations_as_list, device=None, crop_idx=None):
    ...
    if "." in unit:
        ...
        if crop_idx is not None:
            tensor_input = tensor_input[crop_idx].unsqueeze(0)
...
def scatter_neurons(
    ...
    if unit in {"t"}:
        ...
        if use_fast:
            ...
            #if len(unit_locations.shape) == 2:  #@ALEXA added this
            #    unit_locations = unit_locations[0].to(dtype=torch.int64)
            if len(unit_locations[0]) > 0:  # @alexa added this
                if crop_idx is not None:
                    bb = crop_idx
                else:
                    bb = _batch_idx
                tensor_input[
                    bb, unit_locations, start_index:end_index
                ] = replacing_tensor_input
        return tensor_input
